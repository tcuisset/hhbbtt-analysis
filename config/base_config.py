...
def get_common_processes():
    ...
    processes = [
        ...
        Process("ggZH", Label("ggZH"), color=(130, 39, 197), parent_process="higgs"),
        # WH_htt
...
class BaseConfig(cmt_config):
    ...
    def add_features(self):
        flat_ntuples = True # set to true to not use Jet_ collection but only bjet1/2_pt/... variables (for lightweight categorization)
        ...
        features = [
            ...
            Feature("bjet1_pt_high", "bjet1_pt" if flat_ntuples else "Jet_pt.at(bjet1_JetIdx)", binning=(30, 20, 500),
                x_title=Label("b_{1} p_{T}"), selection="bjet1_JetIdx>=0",
                units="GeV", tags=["base"],
                **self.jet_systs_params),
            Feature("bjet1_eta", "bjet1_eta" if flat_ntuples else "Jet_eta.at(bjet1_JetIdx)", binning=(20, -3., 3.),
                x_title=Label("b_{1} #eta"), selection="bjet1_JetIdx>=0", tags=["base"]),
            Feature("bjet1_phi", "bjet1_phi" if flat_ntuples else "Jet_phi.at(bjet1_JetIdx)", binning=phi_binning,
                x_title=Label("b_{1} #phi"), selection="bjet1_JetIdx>=0", tags=["base"]),
            Feature("bjet1_mass", "bjet1_mass" if flat_ntuples else "Jet_mass.at(bjet1_JetIdx)", binning=(20, 0, 60),
                x_title=Label("b_{1} m"), selection="bjet1_JetIdx>=0",
                units="GeV", tags=["base"],
                **self.jet_systs_params),
            Feature("bjet1_btagScore", "bjet1_btagDeepFlavB" if flat_ntuples else "Jet_btagDeepFlavB.at(bjet1_JetIdx)", binning=(30,0,1),
                x_title=Label("b-tagging score of b-jet 1"), selection="bjet1_JetIdx>=0", tags=["base"]),
            Feature("bjet2_pt", "bjet2_pt" if flat_ntuples else "Jet_pt.at(bjet2_JetIdx)", binning=(20, 20, 220),
                x_title=Label("b_{2} p_{T}"),
                units="GeV", tags=["base"], selection="bjet2_JetIdx>=0",
                **self.jet_systs_params),
            Feature("bjet2_pt_high", "bjet2_pt" if flat_ntuples else "Jet_pt.at(bjet2_JetIdx)", binning=(30, 20, 500),
                x_title=Label("b_{2} p_{T}"),
                units="GeV", tags=["base"], selection="bjet2_JetIdx>=0",
                **self.jet_systs_params),
            Feature("bjet2_eta", "bjet2_eta" if flat_ntuples else "Jet_eta.at(bjet2_JetIdx)", binning=(20, -3., 3.),
                x_title=Label("b_{2} #eta"), selection="bjet2_JetIdx>=0", tags=["base"]),
            Feature("bjet2_phi", "bjet2_phi" if flat_ntuples else "Jet_phi.at(bjet2_JetIdx)", binning=phi_binning,
                x_title=Label("b_{2} #phi"), selection="bjet2_JetIdx>=0", tags=["base"]),
            Feature("bjet2_mass", "bjet2_mass" if flat_ntuples else "Jet_mass.at(bjet2_JetIdx)", binning=(20, 0, 60),
                x_title=Label("b_{2} m"), selection="bjet2_JetIdx>=0",
                units="GeV", tags=["base"],
                **self.jet_systs_params),
            Feature("bjet2_btagScore", "bjet2_btagDeepFlavB" if flat_ntuples else "Jet_btagDeepFlavB.at(bjet2_JetIdx)", binning=(30,0,1),
            ...
            # fatjet features
            Feature("fatjet_pt", "fatjet_pt" if flat_ntuples else "FatJet_pt.at(fatjet_JetIdx)", binning=(20, 200, 500),
                x_title=Label("FatJet p_{T}"), selection="fatjet_JetIdx>=0",
                units="GeV", tags=["base"],
                **self.jet_systs_params),
            Feature("fatjet_pt_high", "fatjet_pt" if flat_ntuples else "FatJet_pt.at(fatjet_JetIdx)", binning=(30, 200, 3000),
                x_title=Label("FatJet p_{T}"), selection="fatjet_JetIdx>=0",
                units="GeV", tags=["base"],
                **self.jet_systs_params),
            Feature("fatjet_eta", "fatjet_eta" if flat_ntuples else "FatJet_eta.at(fatjet_JetIdx)", binning=(20, -3.2, 3.2),
                x_title=Label("FatJet #eta"), selection="fatjet_JetIdx>=0", tags=["base"]),
            Feature("fatjet_phi", "fatjet_phi" if flat_ntuples else "FatJet_phi.at(fatjet_JetIdx)", binning=phi_binning,
                x_title=Label("FatJet #phi"), selection="fatjet_JetIdx>=0", tags=["base"]),
            Feature("fatjet_mass", "fatjet_mass" if flat_ntuples else "FatJet_mass.at(fatjet_JetIdx)", binning=(20, 0, 200),
                x_title=Label("FatJet m"), selection="fatjet_JetIdx>=0",
                units="GeV", tags=["base"],
                **self.jet_systs_params),
            Feature("fatjet_msoftdrop", "fatjet_msoftdrop" if flat_ntuples else "FatJet_msoftdrop.at(fatjet_JetIdx)", binning=(20, 0, 200),
                x_title=Label("FatJet SoftDrop mass"), selection="fatjet_JetIdx>=0",
                units="GeV", tags=["base"],
                ), # TODO softdrop systematics ??
            Feature("fatjet_pnet_XbbVsQCD", "fatjet_particleNetLegacy_Xbb/(fatjet_particleNetLegacy_Xbb+fatjet_particleNetLegacy_QCD)" if flat_ntuples else "FatJet_particleNetLegacy_Xbb.at(fatjet_JetIdx)/(FatJet_particleNetLegacy_Xbb.at(fatjet_JetIdx)+FatJet_particleNetLegacy_QCD.at(fatjet_JetIdx))", binning=(40, 0, 1),
                x_title=Label("FatJet ParticleNet XbbVsQCD"), selection="fatjet_JetIdx>=0",
                tags=["base"],
                ),
            Feature("fatjet_pnet_XbbVsQCD_near1", "fatjet_particleNetLegacy_Xbb/(fatjet_particleNetLegacy_Xbb+fatjet_particleNetLegacy_QCD)" if flat_ntuples else "FatJet_particleNetLegacy_Xbb.at(fatjet_JetIdx)/(FatJet_particleNetLegacy_Xbb.at(fatjet_JetIdx)+FatJet_particleNetLegacy_QCD.at(fatjet_JetIdx))", binning=(40, self.boosted_bb_tagging_wp, 1),
                x_title=Label("FatJet ParticleNet XbbVsQCD"), selection="fatjet_JetIdx>=0",
                tags=["base"],
                ),
        ...
        features.extend([
            ...
            *( # features only for boostedTaus
            ...
            Feature("lep2_mass", "dau2_mass", binning=(30, -0.5, 3),
            ...
            *( # features only for boostedTaus
            ...
            Feature("lep2_rawAntiEle2018", "boostedTau_rawAntiEle2018[dau2_idx]", binning=(50, -1, 1),
                ...
                tags=["study"]),
            Feature("lep2_idAntiMu", "boostedTau_idAntiMu[dau2_idx]", binning=(3, 0, 3),
